...
class InstrumentSpecTemplateTests(unittest.TestCase):
    def test_instrument_spec_template_prefers_route_graph_dto_inputs(self) -> None:
        ...
        self.assertIn("route_renderables or optical_path.routes or optical_path.light_paths", template)
        ...
        self.assertIn("node.inventory_display_number or node.display_number", template)
        ...
        self.assertNotIn("hardware_index_by_inventory_id.get(item.id)", template)
