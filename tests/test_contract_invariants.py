...
class ContractInvariantTests(unittest.TestCase):
    ...
    def test_plan_experiments_prompt_uses_canonical_v2_route_language(self) -> None:
        ...
        self.assertIn("Treat llm_context.authoritative_route_contract as route truth", rendered)
        self.assertIn("Procedure:", rendered)
        self.assertIn("Map them onto recorded routes and verify route membership", rendered)
        ...
        self.assertIn("Never combine components from different routes", rendered)
        self.assertIn("recorded fact, deduction, expert inference and unknown", rendered)
        self.assertIn("relevant detectors/endpoints", rendered)
        self.assertIn("Objectives are instrument-level", rendered)
        self.assertIn("Preserve branches, splitters and endpoints", rendered)
