...
- `scripts/`
  - `scripts/dashboard_builder.py` — compatibility/CLI shim; implementations live in `scripts/dashboard/*`.
  - `scripts/validate.py` — compatibility façade and module-execution shim; implementations live in `scripts/validation/*`.
...
- `scripts/validation/*` enforces that contract and reports migration/deprecation issues (via `scripts/validate.py` compatibility façade).
- `scripts/lightpath/*` builds the consumer DTO from canonical YAML (through `scripts/light_path_parser.py` compatibility shim).
...
- Consumer layers (`scripts/dashboard/*`, methods generator, instrument spec pages, virtual microscope runtime/app, audit tooling) must treat the DTO as their contract rather than re-infering topology from raw YAML.
