"""Historical-transcript compatibility wrapper for the 116-scenario replay."""
...
def historical_pick_control(
    page: Page, target: str, section: str, want_checked: bool
) -> tuple[str, str, float]:
    try:
        return _ORIGINAL_PICK_CONTROL(page, target, section, want_checked)
    except RuntimeError:
        # The original LSM 510 audit logged one detector as "Zeiss — Zeiss".
        # PR #460 now renders the same recorded component as "Unknown PMT — Zeiss".
        # Resolve this only when the historical label is a repeated manufacturer and
        # exactly one visible control in that section belongs to that manufacturer.
