(function (root, factory) {
  ...
  function normalizeTypeToken(spectrum) {
    const raw = [
      ...
      .trim();
    if (!raw) return '';
    if (/(^|\b)(2p|two photon|two photons|two-photon|multiphoton|multi photon|multi-photon)(\b|$)/.test(raw)) return 'ex2p';
    if (/(^|\b)(em|emi|emission|fluorescence)(\b|$)/.test(raw)) return 'em';
    if (/(^|\b)(ex|exc|excitation|absorption|absorbance|1p|one photon|one-photon)(\b|$)/.test(raw)) return 'ex1p';
    return raw;
  }

  function matchSpectrumType(token, aliases) {
    const normalized = cleanString(token).toLowerCase();
    return (Array.isArray(aliases) ? aliases : [aliases]).some((alias) => normalized === alias || normalized.includes(alias));
  }

  function collectSpectraContainers(detail, state) {
    const containers = [];
    const enqueue = (owner, scope) => {
      if (!owner || typeof owner !== 'object') return;
      [owner.spectra, owner.spectrum, owner.spectral_data, owner.spectra_data, owner.values].forEach((spectra) => {
        if (Array.isArray(spectra) && spectra.length) {
          containers.push({ owner, scope, spectra });
        }
      });
    };
    if (state && typeof state === 'object') enqueue(state, 'state');
    if (detail && typeof detail === 'object') {
      enqueue(detail, 'detail');
      if (detail.default_state && typeof detail.default_state === 'object') enqueue(detail.default_state, 'default_state');
      if (!state && Array.isArray(detail.states)) {
        detail.states.forEach((entry) => enqueue(entry, 'state'));
      }
    }
    return containers;
  }

  function pointsFromSpectrumRecord(record) {
    return normalizePoints(record && (record.data || record.points || record.values || record.spectrum || record.curve));
  }

  function extractSpectra(detail, state) {
    const extracted = { ex1p: [], ex2p: [], em: [] };
    collectSpectraContainers(detail, state).forEach((container) => {
      (Array.isArray(container.spectra) ? container.spectra : []).forEach((spectrum) => {
        const token = normalizeTypeToken(spectrum);
        const points = pointsFromSpectrumRecord(spectrum);
        if (!points.length) return;
        if (token === 'ex2p' && !extracted.ex2p.length) {
          extracted.ex2p = points;
          return;
        }
        if (token === 'em' && !extracted.em.length) {
          extracted.em = points;
          return;
        }
        if (token === 'ex1p' && !extracted.ex1p.length) {
          extracted.ex1p = points;
        }
      });
    });
    return extracted;
  ...
  function normalizeFPbaseSpectraResponse(data) {
    ...
    const enqueue = (row) => {
      ...
      if (!points.length) return;
      const type = normalizeTypeToken(row);
      if (!['ex1p', 'ex2p', 'em'].includes(type)) return;
  ...
  function stateRecordFromRaw(state, index, detail, fallbackSummary) {
    ...
    const extracted = extractSpectra(detail, state);
    const ex1p = extracted.ex1p;
    const ex2p = extracted.ex2p;
    const em = extracted.em;
